...
for deviceIndex in ${!allDeviceArray[*]}
	do
		...
		echo "uninstall apk..."
		adb -s ${allDeviceArray[deviceIndex]} uninstall com.bmbstack.kit.app
		echo "uploading apk..."
		adb -s ${allDeviceArray[deviceIndex]} install app/python/outputs_apk/app-istormcity-release.apk
		echo "startup apk..."
		adb -s ${allDeviceArray[deviceIndex]} shell am start -n com.bmbstack.kit.app/com.bmbstack.kit.app.MainActivity
	done
