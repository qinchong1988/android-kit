...
public class Base64 {
  ...
  public static String encode(byte[] byteData, String encoding)
  ...
  public final static byte[] _encode(byte[] byteData) {
    ...
    for (iSrcIdx = 0, iDestIdx = 0; iSrcIdx < byteData.length - 2; iSrcIdx += 3) {
      ...
      byteDest[iDestIdx++] =
          (byte) ((byteData[iSrcIdx + 2] >>> 6) & 003 | (byteData[iSrcIdx + 1] << 2) & 077);
      byteDest[iDestIdx++] = (byte) (byteData[iSrcIdx + 2] & 077);
    }
  ...
  public final static byte[] decode(String encoded, String encoding)
  ...
  public final static byte[] _decode(byte[] byteData) throws IllegalArgumentException {
    ...
    byte[] byteTemp = new byte[byteData.length];
         ...
         */
    for (reviSrcIdx = byteData.length; reviSrcIdx - 1 > 0 && byteData[reviSrcIdx - 1] == '=';
        reviSrcIdx--) {
    ...
    for (iSrcIdx = 0; iSrcIdx < reviSrcIdx; iSrcIdx++) {
      if (byteData[iSrcIdx] == '+') {
         ...
         * example
         */
    for (iSrcIdx = 0, iDestIdx = 0; iSrcIdx < reviSrcIdx && iDestIdx < ((byteDest.length / 3) * 3);
        iSrcIdx += 4) {
      byteDest[iDestIdx++] =
      ...
      byteDest[iDestIdx++] =
          (byte) ((byteTemp[iSrcIdx + 2] << 6) & 0xC0 | byteTemp[iSrcIdx + 3] & 0x3F);
    }
    ...
    if (iSrcIdx < reviSrcIdx) {
      if (iSrcIdx < reviSrcIdx - 2) {
        ...
        throw new IllegalArgumentException(
            "Warning: 1 input bytes left to process. This was not Base64 input");
      }
    }
    return byteDest;
  }
