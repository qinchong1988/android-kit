...
public class AppUtils {
  ...
  /**
   * 静默安装App
   * <p>非root需添加权限 {@code <uses-permission android:name="android.permission.INSTALL_PACKAGES"
   * />}</p>
   *
   * @param filePath 文件路径
  ...
  /**
   * 静默卸载App
   * <p>非root需添加权限 {@code <uses-permission android:name="android.permission.DELETE_PACKAGES"
   * />}</p>
   *
   * @param context 上下文
   * @param packageName 包名
   ...
   */
  public static boolean uninstallAppSilent(Context context, String packageName,
      boolean isKeepData) {
  ...
  public static boolean isAppRoot() {
    ShellUtils.CommandResult result =
        ShellUtils.execCmd("echo root", true);
    if (result.result == 0) {
      return true;
    }
  ...
  public static void getAppDetailsSettings(Context context, String packageName) {
    ...
    context.startActivity(
        IntentUtils.getAppDetailsSettingsIntent(packageName));
  }
   ...
   * @return App签名
   */
  @SuppressLint("PackageManagerGetSignatures") public static Signature[] getAppSignature(
